...
function greeting(name, gender, isSelf) {
  // do stuff here ...
    ...
    if(gender === "male"){
        ...
        return  `${firstLetterUpperCase(greetingWords[1])}, ${prefixes[3]} name are ${name}`; 
    }
      return  `${firstLetterUpperCase(greetingWords[i])}, ${prefixes[i]} name is ${name}`; // use template literal for string to return
}
